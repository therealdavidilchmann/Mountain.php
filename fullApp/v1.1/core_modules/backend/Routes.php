<?php
    ...
    class Routes {
        ...
        public function restrictedArea($area)
        {
            ...
            Response::error(new HTTP_Error(500, "Restricted area '" . $area . "' not defined."));
        }
                ...
                if ($this->routes[$i]->route == $path) {
                    if ($_SERVER['REQUEST_METHOD'] == $this->routes[$i]->method) {
                        if ($this->routes[$i]->authRequired) {
                            if (Auth::isLoggedIn()) {
                                if (!empty($this->routes[$i]->restrictionName)) {
                                    if (Auth::userHasAccessTo($this->routes[$i]->restrictionName)) {
                                        return $this->getController($this->routes[$i], $application);
                                    } else {
                                        return Response::error(new HTTP_Error(HTTP_Error::FORBIDDEN));
                                    }
                                } else {
                                    return $this->getController($this->routes[$i], $application);
                                }
                            ...
                            if (!empty($this->routes[$i]->restrictionName)) {
                                return Response::error(new HTTP_Error(500, "Route has restriction, but you don't have to be logged in."));
                            } else {
                                return $this->getController($this->routes[$i], $application);;
                            }
                        }
        ...
        public function addRestrictedArea($area)
        {
            $this->restrictedAreas[$area] = $area;
        }
        ...
        public function getController(Route $route, $application)
        {
            if (is_string($route->activator)) {
                ...
                if (file_exists("../controllers/$activator[0].php")) {
                    ...
                    $method = $activator[1];
                    return $class->$method($application);
                } else {
                    return Response::error(new HTTP_Error(500, "Controller $activator[0] doesn't exist."));
                }
                exit;
            } else if (is_array($route->activator)) {
                return Response::view($route->activator[0]);
                exit;
            } else {
        ...
        {
            $this->addRestrictedArea("usermanagement");
